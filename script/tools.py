...
def update_1(end_idx, sr_list, interval):
    ...
    plt.xlabel('Episodes')
    plt.ylabel("KDE")
    plt.xticks(np.arange(100, end_idx*interval+10, 100))  # 设置x轴刻度值的字体大小
...
def analysis_actions():
    ...
    ani = animation.FuncAnimation(fig, update_1, list(range(1, times+1)),
                                  interval=1000, fargs=(sr_list, interval))
    ani.save('target_1.gif', writer='imagemagick', fps=60)

    # target 2
    bins = max(many_cmd_count)
    times = len(many_cmd_count) // interval + 1
    ani = animation.FuncAnimation(fig, update_2, list(range(0, times)),
                                  interval=1000, fargs=(many_cmd_count, bins, interval))
    ani.save('target_2.gif', writer='imagemagick', fps=60)
    # plt.show()
